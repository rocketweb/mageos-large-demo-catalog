...
class IsolatedRootTest(unittest.TestCase):
    ...
    def test_magento_host_format_and_no_source_credentials(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            (source/'app/autoload.php').write_text('<?php $composerAutoloader = include $vendorAutoload;')
            ...
            metadata=json.loads((out/'composer.json').read_text())
            self.assertFalse(metadata['config']['allow-plugins'])
            lock=json.loads((out/'composer.lock').read_text())
            self.assertEqual(lock['packages'][0]['version'],'3.5.0')
            self.assertIn('isolateRehearsalAutoloader',(out/'app/autoload.php').read_text())
            self.assertTrue(json.loads((out/'isolation.json').read_text())['source_generated_code_excluded'])
            candidate=Path(tmp)/'candidate'
            build(argparse.Namespace(source=source,output_dir=candidate,database='wands_rehearsal_test',php=str(PHP),candidate_label_plugin=True))
            info=json.loads((candidate/'isolation.json').read_text())
            self.assertTrue(info['candidate_label_plugin'])
            self.assertEqual(len(info['candidate_hashes']),2)
            self.assertTrue((candidate/'app/code/WandsRehearsal/CandidateLabel/etc/frontend/di.xml').is_file())
            self.assertIn('WandsRehearsal_CandidateLabel',(candidate/'app/etc/config.php').read_text())
